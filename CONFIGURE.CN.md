...
| `candidate_expand_on_paging` | 翻页时展开多行候选 | `true` / `false` | `false` | 按上下箭头或向后翻页即可展开，最多同时显示三页；上下箭头和翻页键切换行并保留选中编号，目标行不足时选中最后一项，左右箭头仍逐项移动。第一页按上箭头也会展开，但不循环到末页。仅当前行显示数字，其他行保留数字占位，切换行时字词位置不变。数字选词只作用于当前行，Tab 区保持不变。 |
