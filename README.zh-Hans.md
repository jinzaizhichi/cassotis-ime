...
统计先验经过量化后写入本地词库数据库，大部分紧凑重排器导出为确定性的原生 Pascal 参数。v1.18.0 的续写后备层采用 INT8 权重量化、混合精度 ONNX 模型；ONNX Runtime 只由外部 host 进程加载，不进入 TSF DLL。运行时仍然只执行本地、规模受控的计算，不需要联网或 GPU；模型尚未加载或不可用时直接沿用原有结果。长句与短词使用彼此独立的排序路径，一条路径的改进不会替代另一条路径的匹配规则。
