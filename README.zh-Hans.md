...
- 已实现候选窗、翻页、选词与上屏流程。
- 支持言泉原创的一键补全：候选窗每次只显示一个可信续写方案，按配置键即可补全；依次优先采用用户词、基础词库 exact 词和离线复核的强转移补全。
...
## 长句一键补全基准测试-16300
该基准在固定长句语料中保留末尾四个完整拼音音节不输入，评估界面实际显示的唯一补全方案。测试采用严格的单参考答案评分，合理但不同于原文的续写仍计为未命中；完整口径见 [BENCHMARK.CN.md](BENCHMARK.CN.md)。
