...
## 主要特色
- TSF 文字服務主流程已可用（註冊、啟用、組合生命週期），並註冊 TSF COM-less 能力類別，以支援採用該啟用方式的宿主。
- TSF DLL 同時支援 Win64 與 Win32（`svr.dll` / `svr32.dll`），host 處理程序僅保留 Win64。
- 已實作候選視窗、換頁、選詞與送出文字流程。
- 支援言泉原創的一鍵補全：候選視窗每次只顯示一個可信的續寫方案，按下設定鍵即可補全；依序優先採用使用者詞、基礎詞庫 exact 詞及離線複核的強轉移補全。長句靜態補全未命中時，受詞庫約束的本機模型可非同步複核 exact 後綴路徑，信心不足時不顯示提示。
...
Cassotis v1.15.0 將長句 Top1/Top2 的多階段調整收斂為語料訓練驅動的最終裁決，並擴充短詞前後文重排證據；只有學習所得的優勢足夠明確時才調整順序。

Cassotis v1.18.0 新增受詞庫約束的長句局部續寫神經後備層。這個 1,382 萬參數的模型最多比較 32 條完全由詞庫 exact 詞組成的後綴路徑，只輸出一至三個詞、最多六個音節，或主動放棄提示。現有靜態補全仍是零額外開銷的第一層；只有靜態未命中的請求才會非同步交給 `cassotis_ime_host.exe` 的背景 CPU 執行緒，過期或低信心結果會被捨棄。

Cassotis v1.19.0 將拼音條件序列評分器接入長句最終裁決：模型在外部 host 中聯合比較完整拼音與最多 16 條穩定候選，並由離線訓練的門控及融合模型決定是否調整順序。長句一鍵補全亦擴充多層級後綴召回索引，先由原生召回選擇器篩選 exact 續寫路徑，再交由既有補全模型複核。

Cassotis v1.20.0 在既有排序流程中加入文件層級的前後文自適應與受約束生成。游標前的文件快照會在記憶體中提供臨時術語與轉移證據；量化生成模型則補充靜態召回未涵蓋的拼音對齊完整候選與長句局部續寫，信心不足或模型不可用時維持原有結果。

Cassotis v1.22.0 加入拼音約束的局部校正與跨句語意。由中文預訓練模型蒸餾而來的本機模型，可結合目前句子、原始拼音及最多約 256 字的可用前文，審慎校正簡體全拼長句中的少量同音字詞，不再侷限於既有候選之間的換位；使用者詞、完整詞庫精確比對及已確認文字仍受保護。

Cassotis v1.26.0 將長句局部校正從逐字判斷擴充為少量完整修正方案的聯合選擇。以獨立語料訓練的模型會結合修改片段及其前後文，複核原有結果與修正方案，證據不足時保留原有結果，以減少誤改。
...
統計先驗經過量化後寫入本機詞庫資料庫，大部分精簡重排器會匯出為具確定性的原生 Pascal 參數。v1.18.0 的續寫後備層、v1.19.0 的拼音條件評分器、v1.20.0 的受約束候選與續寫生成器及 v1.22.0 的局部校正模型均以量化 ONNX 模型部署；ONNX Runtime 只由外部 host 處理程序載入，不會進入 TSF DLL。執行階段仍然只會在本機進行規模受控的運算，不需要網路連線或 GPU；模型尚未載入或無法使用時會直接沿用原有結果。長句與短詞使用彼此獨立的排序路徑，其中一條路徑的改進不會取代另一條路徑的比對規則。
...
## 一鍵補全前後文基準測試-12831
這項基準沿用固定的短詞前後文語料，將符合條件的目標展開為 12,831 個逐音節拼音前綴機會，評估啟用左側前文時介面實際顯示的唯一補全方案。

公開結果只保留 `補全命中率`、`平均節省按鍵`、`逐鍵穩定率` 與 `P95 (ms)` 四欄。各版本資料從 `v1.15.0` 開始記錄；案例建置、評分與延遲測量方式請參閱 [BENCHMARK.CN.md](BENCHMARK.CN.md)。

| 版本 | 補全命中率 | 平均節省按鍵 | 逐鍵穩定率 | P95 (ms) |
| --- | --- | --- | --- | --- |
...
## 長句一鍵補全基準測試-16300
這項基準在固定長句語料中保留末尾四個完整拼音音節不輸入，評估介面實際顯示的唯一補全方案。顯示結果只要正確延伸目標前綴，且仍是參考句的前綴即可命中，不要求一次補到整句末尾；完整測試方式請參閱 [BENCHMARK.CN.md](BENCHMARK.CN.md)。

### 現行局部續寫方式

| 版本 | 局部續寫命中率 | 提示涵蓋率 | 總節省按鍵 | P95 (ms) |
| --- | --- | --- | --- | --- |
...
`提示涵蓋率`統計實際顯示了任一補全方案的機會數。`總節省按鍵`累加所有正確局部續寫命中的淨節省按鍵，並已為每次接受補全扣除一個按鍵。單次命中平均節省按鍵與逐鍵穩定率仍保留在詳細診斷報告中，不再用於公開橫向比較。

### 歷史整句嚴格命中方式

| 版本 | 整句嚴格命中率 | 總節省按鍵 | P95 (ms) |
| --- | --- | --- | --- |
| `v1.17.0` | 20/16300 (0.12%) | 124 | 55.049 |
| `v1.16.0` | 10/16300 (0.06%) | 71 | 65.063 |

`v1.16.0` 與 `v1.17.0` 採用舊的整句嚴格命中方式，僅作為歷史結果保留，不能與現行局部續寫方式直接比較。
