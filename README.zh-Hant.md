...
統計先驗經過量化後寫入本機詞庫資料庫，大部分精簡重排器會匯出為具確定性的原生 Pascal 參數。v1.18.0 的續寫後備層、v1.19.0 的拼音條件評分器及 v1.20.0 的受約束候選與續寫生成器均以量化 ONNX 模型部署；ONNX Runtime 只由外部 host 處理程序載入，不會進入 TSF DLL。執行階段仍然只會在本機進行規模受控的運算，不需要網路連線或 GPU；模型尚未載入或無法使用時會直接沿用原有結果。長句與短詞使用彼此獨立的排序路徑，其中一條路徑的改進不會取代另一條路徑的比對規則。
