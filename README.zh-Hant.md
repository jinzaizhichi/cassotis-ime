...
- 支援候選換頁、選詞與送出文字；可在設定中開啟多行展開模式，換頁時最多顯示三行候選，數字鍵選取目前行的詞，Tab 補全區不受影響。
...
<p align="center">
  <img src="snapshot_multiplelines.png" alt="三行展開的候選視窗，第二行為目前行" width="532" height="153">
  <br>
  <em>三行展開的候選視窗，目前行顯示選詞編號。</em>
</p>
...
| 版本 | 局部續寫命中率 | 預測提示涵蓋率 | 總節省按鍵 | P95 (ms) |
| --- | --- | --- | --- | --- |
...
`預測提示涵蓋率`統計實際顯示預測補全方案的機會數，不包含僅轉換已輸入拼音的詞庫拼接備援結果。`總節省按鍵`累加所有正確局部續寫命中的淨節省按鍵，並已為每次接受補全扣除一個按鍵。單次命中平均節省按鍵與逐鍵穩定率仍保留在詳細診斷報告中，不再用於公開橫向比較。
