...
- 支援言泉原創的一鍵補全：候選視窗每次只顯示一個可信的續寫方案，按下設定鍵即可補全；依序優先採用使用者詞、基礎詞庫 exact 詞及離線複核的強轉移補全。
...
## 長句一鍵補全基準測試-16300
這項基準在固定長句語料中保留末尾四個完整拼音音節不輸入，評估介面實際顯示的唯一補全方案。測試採用嚴格的單一參考答案評分，合理但不同於原文的續寫仍計為未命中；完整測試方式請參閱 [BENCHMARK.CN.md](BENCHMARK.CN.md)。
