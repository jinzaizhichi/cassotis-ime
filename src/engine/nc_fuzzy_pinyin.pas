...
type
    ...
    TncFuzzyPinyinSyllableVariants = TArray<TncFuzzyPinyinSyllableVariant>;
    ...
    TncFuzzyPinyinQueryVariants = TArray<TncFuzzyPinyinQueryVariant>;
...
begin
    ...
    begin
        if SameText(normalized, nc_fuzzy_pinyin_rule_name(candidate)) then
        begin
            rule := candidate;
            Exit(True);
        end;
...
var
    ...
    begin
        ...
        begin
            if SameText(Result[existing_index].text, key) then
            begin
...
begin
    ...
    begin
        ...
        begin
            if SameText(initial_value, pair.left_value) then
            begin
                append_part_variant(initial_alternatives, pair.right_value,
                    pair.rule);
            end
            else if SameText(initial_value, pair.right_value) then
            begin
        ...
        begin
            if SameText(final_value, pair.left_value) then
            begin
                append_part_variant(final_alternatives, pair.right_value,
                    pair.rule);
            end
            else if SameText(final_value, pair.right_value) then
            begin
    ...
    begin
        ...
        while (sort_idx >= 0) and
            ...
            (CompareText(Result[sort_idx].text, sorted_item.text) > 0))) do
        begin
...
type
    ...
    TncQueryBuildStates = TArray<TncQueryBuildState>;
var
    normalized: string;
    ...
    begin
        ...
        begin
            if SameText(target[target_idx].text, value.text) then
            begin
                Exit;
            end;
        end;
    ...
    begin
        ...
        begin
            ...
            while (insert_idx >= 0) and
                ...
                (CompareText(values[insert_idx].text, value.text) > 0))) do
            begin
...
begin
    ...
    if not SameText(reconstructed,
        StringReplace(normalized, '''', '', [rfReplaceAll])) then
    begin
        Exit;
    end;
