...
void AppendBaseCandidates(const LocalCompletionIndex& index, int base_rank,
    const wchar_t* encoded_path, std::vector<LocalRuntimeCandidate>& output) {
    ...
    for (int width = std::min<int>(3, static_cast<int>(words.size()));
        ...
        const auto encoded_anchor = WideToUtf8(anchor);
        const auto* row = index.FindAnchor(encoded_anchor);
        if (row == nullptr) {
            continue;
        }
