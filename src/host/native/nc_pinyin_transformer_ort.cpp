...
bool LoadParallelAllowed(const wchar_t* path, ParallelGeneratorHandle& handle,
    std::wstring& error) {
    std::ifstream stream(path, std::ios::binary);
    if (!stream) {
        error = L"cannot open pinyin generator constraint table";
        return false;
    }
    std::array<char, 8> magic{};
    if (!stream.read(magic.data(), static_cast<std::streamsize>(magic.size())) ||
        std::memcmp(magic.data(), "CASPGA01", magic.size()) != 0) {
        error = L"invalid pinyin generator constraint table";
        return false;
    }
    uint32_t version = 0;
    if (!ReadBinary(stream, version) || version != 1 ||
        ...
        error = L"unsupported pinyin generator constraint table";
        return false;
    }
    ...
    if (!stream.read(reinterpret_cast<char*>(handle.allowed_counts.data()),
        ...
        error = L"truncated pinyin generator constraint table";
        return false;
    }
    for (const uint16_t count : handle.allowed_counts) {
        if (count > handle.allowed_limit) {
            error = L"invalid pinyin generator constraint count";
            return false;
        }
    }
    return true;
...
extern "C" __declspec(dllexport) int __cdecl nc_pt_run_conditional(
    void* opaque_handle,
    const int64_t* char_ids,
    const int64_t* pinyin_ids,
    const uint8_t* candidate_mask,
    ...
    if (opaque_handle == nullptr || char_ids == nullptr || pinyin_ids == nullptr ||
        ...
        SetError(error_text, error_capacity, L"invalid conditional inference arguments");
        return 0;
    }
    try {
        FloatingPointMaskGuard floating_point_guard;
        ...
        if (outputs.size() != 1 || !outputs[0].IsTensor()) {
            SetError(error_text, error_capacity,
                L"conditional model returned an invalid output tensor");
            return 0;
        }
        const auto info = outputs[0].GetTensorTypeAndShapeInfo();
        if (info.GetElementCount() < kConditionalCandidateCount) {
            SetError(error_text, error_capacity,
                L"conditional model output tensor is too small");
            return 0;
        }
        const float* scores = outputs[0].GetTensorData<float>();
        ...
        SetError(error_text, error_capacity, L"unknown conditional inference failure");
    }
    return 0;
}
...
extern "C" __declspec(dllexport) void* __cdecl nc_pg_create(
    const wchar_t* model_path,
    const wchar_t* allowed_path,
    int intra_threads,
    wchar_t* error_text,
    int error_capacity) {
    SetError(error_text, error_capacity, L"");
    if (model_path == nullptr || *model_path == L'\0' ||
        allowed_path == nullptr || *allowed_path == L'\0') {
        SetError(error_text, error_capacity, L"pinyin generator path is empty");
        return nullptr;
    }
    try {
        FloatingPointMaskGuard floating_point_guard;
        ...
        SetError(error_text, error_capacity,
            L"unknown pinyin generator initialization failure");
    }
    return nullptr;
}

extern "C" __declspec(dllexport) int __cdecl nc_pg_run(
    void* opaque_handle,
    const int64_t* pinyin_ids,
    int syllable_count,
    int beam_size,
    ...
    if (opaque_handle == nullptr || pinyin_ids == nullptr ||
        ...
        SetError(error_text, error_capacity,
            L"invalid pinyin generator inference arguments");
        return 0;
    }
    try {
        FloatingPointMaskGuard floating_point_guard;
        ...
        if (!handle->session) {
            SetError(error_text, error_capacity,
                L"pinyin generator session is unavailable");
            return 0;
        }
        ...
        for (int position = 0; position < syllable_count; ++position) {
            if (input_ids[position] < 0 ||
                ...
                SetError(error_text, error_capacity,
                    L"unsupported pinyin generator syllable");
                return 0;
            }
        }
        ...
        if (outputs.size() != 1 || !outputs[0].IsTensor()) {
            SetError(error_text, error_capacity,
                L"pinyin generator returned an invalid output tensor");
            return 0;
        }
        const auto info = outputs[0].GetTensorTypeAndShapeInfo();
        ...
        if (info.GetElementCount() < expected) {
            SetError(error_text, error_capacity,
                L"pinyin generator output tensor is too small");
            return 0;
        }
        ...
        for (int position = 0; position < syllable_count; ++position) {
            ...
            if (expanded.empty()) {
                SetError(error_text, error_capacity,
                    L"pinyin generator produced no constrained path");
                return 0;
            }
        ...
        *output_count = count;
        ...
        SetError(error_text, error_capacity,
            L"unknown pinyin generator inference failure");
    }
    return 0;
}

extern "C" __declspec(dllexport) void __cdecl nc_pg_destroy(
    void* opaque_handle) {
...
public:
    ...
    bool Open(const wchar_t* path, std::wstring& error) {
        ...
        if (file_ == INVALID_HANDLE_VALUE) {
            file_ = nullptr;
            error = L"cannot open local-completion index";
            return false;
        }
        LARGE_INTEGER size{};
        if (!GetFileSizeEx(file_, &size) || size.QuadPart <= 0) {
            error = L"cannot read local-completion index size";
            Close();
            return false;
        }
        ...
        if (mapping_ == nullptr) {
            error = L"cannot map local-completion index";
            Close();
            return false;
        }
        ...
        if (data_ == nullptr) {
            error = L"cannot view local-completion index";
            Close();
            return false;
        }
        return true;
...
std::wstring Utf8TextToWide(std::string_view value) {
    if (value.empty()) {
        return {};
    }
...
std::vector<uint32_t> WideCodepoints(const wchar_t* value) {
    ...
    while (*cursor != L'\0') {
        uint32_t codepoint = static_cast<uint16_t>(*cursor++);
        if (codepoint >= 0xD800 && codepoint <= 0xDBFF &&
            *cursor >= 0xDC00 && *cursor <= 0xDFFF) {
            const uint32_t low = static_cast<uint16_t>(*cursor++);
...
std::vector<std::wstring> SplitPath(const wchar_t* value) {
...
std::vector<std::string> SplitPinyin(const wchar_t* value) {
    ...
    std::wstring current;
    for (const wchar_t* cursor = value;; ++cursor) {
        if (*cursor == L'\'' || *cursor == L' ' || *cursor == L'\3' ||
            *cursor == L'\0') {
            if (!current.empty()) {
...
public:
    bool Load(const wchar_t* path, std::wstring& error) {
        if (path == nullptr || *path == L'\0') {
            error = L"local-completion index path is empty";
            return false;
        }
        ...
        if (file_.size() < sizeof(LocalCompletionIndexHeader)) {
            error = L"local-completion index is truncated";
            return false;
        }
        ...
        if (!std::equal(kLocalCompletionMagic.begin(), kLocalCompletionMagic.end(),
            ...
            error = L"local-completion index format is unsupported";
            return false;
        }
        ...
        if (header_->max_input_length != kLocalCompletionInputLength ||
            ...
            error = L"local-completion index metadata is invalid";
            return false;
        }
    ...
    bool ValidateSorted(std::wstring& error) const {
        ...
        for (uint32_t index = 1; index < header_->pinyin_count; ++index) {
            ...
            if (Text(previous.text_offset, previous.text_length) >=
                ...
                error = L"local-completion pinyin index is not sorted";
                return false;
            }
        }
        for (uint32_t index = 0; index < header_->anchor_count; ++index) {
            ...
            if (row.candidate_start > header_->candidate_count ||
                ...
                error = L"local-completion anchor candidate range is invalid";
                return false;
            }
            if (index > 0) {
                ...
                if (Text(previous.text_offset, previous.text_length) >=
                    ...
                    error = L"local-completion anchor index is not sorted";
                    return false;
                }
            }
        }
        for (uint32_t index = 0; index < header_->candidate_count; ++index) {
            for (uint16_t word_id : Candidate(index).word_ids) {
                if (word_id >= header_->word_count) {
                    error = L"local-completion candidate word ID is invalid";
                    return false;
                }
            }
        }
...
std::string WideSuffixUtf8(const wchar_t* text, size_t width) {
    const auto codepoints = WideCodepoints(text);
...
std::vector<LocalRuntimeCandidate> BuildBaseCandidates(
    ...
    const wchar_t* base_text, const wchar_t* encoded_path) {
    ...
    for (int width = std::min<int>(3, static_cast<int>(words.size()));
        ...
        for (size_t position = words.size() - width; position < words.size(); ++position) {
            if (!anchor.empty()) {
                anchor.push_back(L'\3');
            }
            anchor.append(words[position]);
        }
        std::string encoded_anchor = WideToUtf8(anchor);
    ...
    if (index.header().version == kLocalCompletionIndexVersion) {
        const auto codepoints = WideCodepoints(base_text);
        for (int width = std::min<int>(6, static_cast<int>(codepoints.size()));
             width >= 1; --width) {
            const std::string anchor = "c:" + WideSuffixUtf8(base_text, width);
...
std::vector<LocalRuntimeCandidate> BuildContextualRepairCandidates(
    const LocalCompletionIndex& index, int base_rank, const wchar_t* base_text,
    const std::vector<std::string>& query_syllables) {
    std::vector<LocalRuntimeCandidate> result;
    std::unordered_map<uint64_t, size_t> positions;
    const auto codepoints = WideCodepoints(base_text);
...
bool BuildLocalRecallPool(const LocalCompletionIndex& index,
    const wchar_t* top1_text, const wchar_t* top1_path,
    const wchar_t* top2_text, const wchar_t* top2_path,
    std::vector<LocalRuntimeCandidate>& candidates) {
    candidates.clear();
    if (top1_text == nullptr || *top1_text == L'\0' ||
        top1_path == nullptr || *top1_path == L'\0') {
        return false;
    }
    AppendBaseCandidates(index, 1, top1_text, top1_path, candidates);
    if (top2_text != nullptr && *top2_text != L'\0' &&
        top2_path != nullptr && *top2_path != L'\0') {
...
bool BuildLocalInputs(const LocalCompletionIndex& index, const wchar_t* context,
    const wchar_t* query_syllables, const wchar_t* top1_text,
    const wchar_t* top1_path, const wchar_t* top2_text, const wchar_t* top2_path,
    bool phonetic_only,
    ...
    AppendTextInput(index, WideCodepoints(context), 12, 1, ids, types);
    ...
    ids.push_back(index.header().separator_id);
    types.push_back(0);
    AppendTextInput(index, WideCodepoints(top1_text), 32, 3, ids, types);
    AppendTextInput(index, WideCodepoints(top2_text), 32, 4, ids, types);
...
std::vector<uint32_t> LocalCandidateSuggestion(
    ...
    const wchar_t* top1_text, const wchar_t* top2_text) {
    const wchar_t* base_text = candidate.base_rank == 2 ? top2_text : top1_text;
    auto result = WideCodepoints(base_text);
    ...
    for (uint16_t word_id : candidate.word_ids) {
        ...
        const std::wstring word = Utf8TextToWide(index.WordText(word_id));
        const auto codepoints = WideCodepoints(word.c_str());
...
bool BuildLocalCrossModelInputs(const LocalCompletionIndex& index,
    const wchar_t* context, const wchar_t* query_syllables,
    const wchar_t* top1_text, const wchar_t* top2_text,
    ...
    AppendTextInput(index, WideCodepoints(context),
    ...
    output.type_ids.push_back(0);
    AppendTextInput(index, WideCodepoints(top1_text),
        kLocalCompletionCrossBaseChars, 3,
        output.input_ids, output.type_ids);
    AppendTextInput(index, WideCodepoints(top2_text),
    ...
    const auto top1 = WideCodepoints(top1_text);
    const auto top2 = WideCodepoints(top2_text);
...
bool BuildLocalCandidateOutput(const LocalCompletionIndex& index,
    const LocalRuntimeCandidate& candidate, std::wstring& suffix_text,
    std::wstring& suffix_pinyin, std::wstring& suffix_path,
    int& replace_units) {
    std::string text;
    std::string pinyin;
    suffix_path.clear();
    for (uint16_t word_id : candidate.word_ids) {
        ...
        if (!suffix_path.empty()) {
            suffix_path.push_back(L'\3');
        }
        suffix_path.append(Utf8TextToWide(word_text));
    }
    if (text.empty() || pinyin.empty()) {
        return false;
    }
    suffix_text = Utf8TextToWide(text);
    suffix_pinyin = Utf8TextToWide(pinyin);
...
void SetWideOutput(wchar_t* destination, int capacity, const std::wstring& value) {
    if (destination == nullptr || capacity <= 0) {
        return;
    }
...
extern "C" __declspec(dllexport) void* __cdecl nc_lc_create(
    const wchar_t* model_path,
    ...
    SetError(error_text, error_capacity, L"");
    if (model_path == nullptr || *model_path == L'\0') {
        SetError(error_text, error_capacity, L"local-completion model path is empty");
        return nullptr;
    }
    try {
        FloatingPointMaskGuard floating_point_guard;
        ...
        SetError(error_text, error_capacity,
            L"unknown local-completion initialization failure");
    }
    return nullptr;
}

extern "C" __declspec(dllexport) int __cdecl nc_lc_build_inputs_for_test(
    void* opaque_handle,
    const wchar_t* context,
    const wchar_t* query_syllables,
    const wchar_t* top1_text,
    const wchar_t* top1_path,
    const wchar_t* top2_text,
    const wchar_t* top2_path,
    int phonetic_only,
...
#if defined(CASSOTIS_EXPERIMENTAL_TOP32_CROSS_RANKER)
extern "C" __declspec(dllexport) int __cdecl nc_lc_build_cross_inputs_for_test(
    void* opaque_handle,
    const wchar_t* context,
    const wchar_t* query_syllables,
    const wchar_t* top1_text,
    ...
    int64_t* output_input_ids,
    int64_t* output_type_ids,
    int64_t* output_candidate_char_ids,
    int64_t* output_candidate_pinyin_ids,
    int64_t* output_candidate_boundary_ids,
    int64_t* output_candidate_source_ids,
    float* output_candidate_features,
    int* output_input_length,
    int* output_candidate_count) {
    if (opaque_handle == nullptr || output_input_ids == nullptr ||
        output_type_ids == nullptr || output_candidate_char_ids == nullptr ||
        output_candidate_pinyin_ids == nullptr ||
        output_candidate_boundary_ids == nullptr ||
        output_candidate_source_ids == nullptr ||
        output_candidate_features == nullptr || output_input_length == nullptr ||
        output_candidate_count == nullptr) {
        return 0;
    }
    try {
        auto* handle = static_cast<LocalCompletionHandle*>(opaque_handle);
        std::array<int64_t, kLocalCompletionInputLength> legacy_input_ids{};
        std::array<int64_t, kLocalCompletionInputLength> legacy_type_ids{};
        std::array<int64_t,
            kLocalCompletionCandidateCount * kLocalCompletionPathWords>
            candidate_ids{};
        std::array<float,
            kLocalCompletionCandidateCount * kLocalCompletionFeatureCount>
            candidate_features{};
        ...
        if (!BuildLocalInputs(handle->index, context, query_syllables,
                top1_text, top1_path, top2_text, top2_path,
                phonetic_only != 0, legacy_input_ids, legacy_type_ids,
                candidate_ids, candidate_features, candidates)) {
            return 0;
        }
        LocalCrossModelInputs cross_inputs;
        ...
        std::copy(cross_inputs.input_ids.begin(), cross_inputs.input_ids.end(),
            output_input_ids);
        std::copy(cross_inputs.type_ids.begin(), cross_inputs.type_ids.end(),
            output_type_ids);
        std::copy(cross_inputs.candidate_char_ids.begin(),
            cross_inputs.candidate_char_ids.end(), output_candidate_char_ids);
        std::copy(cross_inputs.candidate_pinyin_ids.begin(),
            cross_inputs.candidate_pinyin_ids.end(), output_candidate_pinyin_ids);
        std::copy(cross_inputs.candidate_boundary_ids.begin(),
            cross_inputs.candidate_boundary_ids.end(), output_candidate_boundary_ids);
        std::copy(cross_inputs.candidate_source_ids.begin(),
            cross_inputs.candidate_source_ids.end(), output_candidate_source_ids);
        std::copy(candidate_features.begin(), candidate_features.end(),
            output_candidate_features);
        *output_input_length = static_cast<int>(cross_inputs.input_ids.size());
        *output_candidate_count = static_cast<int>(candidates.size());
        return 1;
    } catch (...) {
        return 0;
...
extern "C" __declspec(dllexport) int __cdecl nc_lc_score_recall_selector_for_test(
...
extern "C" __declspec(dllexport) int __cdecl nc_lc_build_recall_pool_for_test(
    void* opaque_handle,
    const wchar_t* top1_text,
    const wchar_t* top1_path,
    const wchar_t* top2_text,
...
extern "C" __declspec(dllexport) int __cdecl nc_lc_run(
    void* opaque_handle,
    const wchar_t* context,
    const wchar_t* query_syllables,
    const wchar_t* top1_text,
    ...
    wchar_t* output_suffix_text,
    int output_suffix_text_capacity,
    wchar_t* output_suffix_pinyin,
    int output_suffix_pinyin_capacity,
    wchar_t* output_suffix_path,
    ...
    int error_capacity) {
    SetError(error_text, error_capacity, L"");
    SetWideOutput(output_suffix_text, output_suffix_text_capacity, L"");
    SetWideOutput(output_suffix_pinyin, output_suffix_pinyin_capacity, L"");
    SetWideOutput(output_suffix_path, output_suffix_path_capacity, L"");
    ...
    if (opaque_handle == nullptr || output_suffix_text == nullptr ||
        ...
        SetError(error_text, error_capacity,
            L"invalid local-completion inference arguments");
        return 0;
    }
    try {
        FloatingPointMaskGuard floating_point_guard;
        ...
        if (!handle->session) {
            SetError(error_text, error_capacity,
                L"local-completion model session is unavailable");
            return 0;
        }
        std::vector<LocalScoredRuntimeCandidate> ranked_candidates;
        ...
        // minimum_confidence is calibrated jointly with the model's explicit
        // ABSTAIN score and can be negative. Requiring best_score to exceed
        // ABSTAIN here would override that calibration at runtime.
        if (confidence < minimum_confidence) {
            return 0;
        }
        const auto& selected = ranked_candidates.front().candidate;
        std::wstring suffix_text;
        std::wstring suffix_pinyin;
        std::wstring suffix_path;
        int replace_units = 0;
        if (!BuildLocalCandidateOutput(handle->index, selected,
                suffix_text, suffix_pinyin, suffix_path, replace_units)) {
            return 0;
        }
        SetWideOutput(output_suffix_text, output_suffix_text_capacity,
            suffix_text);
        SetWideOutput(output_suffix_pinyin, output_suffix_pinyin_capacity,
            suffix_pinyin);
        SetWideOutput(output_suffix_path, output_suffix_path_capacity, suffix_path);
        ...
        *output_confidence = confidence;
        ...
        SetError(error_text, error_capacity,
            L"unknown local-completion inference failure");
    }
    return 0;
}

extern "C" __declspec(dllexport) int __cdecl nc_lc_run_pool(
    void* opaque_handle,
    const wchar_t* context,
    const wchar_t* query_syllables,
    const wchar_t* top1_text,
    ...
    const wchar_t* top2_path,
    int phonetic_only,
    wchar_t* output_suffix_texts,
    int output_suffix_text_stride,
    wchar_t* output_suffix_pinyins,
    int output_suffix_pinyin_stride,
    wchar_t* output_suffix_paths,
    ...
    if (opaque_handle == nullptr || output_suffix_texts == nullptr ||
        ...
        output_scores == nullptr ||
        output_abstain_score == nullptr || output_candidate_count == nullptr ||
        output_suffix_text_stride < 2 || output_suffix_pinyin_stride < 2 ||
        output_suffix_path_stride < 2 || output_capacity <= 0) {
        SetError(error_text, error_capacity,
            L"invalid local-completion pool inference arguments");
        return 0;
    }
    ...
    *output_abstain_score = 0.0f;
    try {
        ...
        if (!ScoreLocalCompletionCandidates(*handle, context,
                query_syllables, top1_text, top1_path, top2_text, top2_path,
                phonetic_only != 0,
                ranked_candidates, abstain_score)) {
            return 0;
        }
        ...
        *output_candidate_count = written;
        ...
        SetError(error_text, error_capacity,
            L"unknown local-completion pool inference failure");
    }
    return 0;
}

extern "C" __declspec(dllexport) void __cdecl nc_lc_destroy(void* opaque_handle) {
...
bool BuildLocalGeneratorInputs(const LocalCompletionIndex& index,
    ...
    std::vector<int64_t> ids{static_cast<int64_t>(index.header().cls_id)};
    std::vector<int64_t> types{0};
    AppendTextInput(index, WideCodepoints(context), 32, 1, ids, types);
    const auto pinyins = SplitPinyin(query_syllables);
...
extern "C" __declspec(dllexport) void* __cdecl nc_lcg_create(
    const wchar_t* model_path, const wchar_t* index_path, int intra_threads,
    wchar_t* error_text, int error_capacity) {
    SetError(error_text, error_capacity, L"");
    try {
        FloatingPointMaskGuard floating_point_guard;
...
extern "C" __declspec(dllexport) int __cdecl nc_lcg_run(
    void* opaque_handle, const wchar_t* context,
    const wchar_t* query_syllables, const wchar_t* top1_text,
    const wchar_t* top2_text, float minimum_confidence,
    wchar_t* output_suffix_text, int output_suffix_text_capacity,
    wchar_t* output_suffix_pinyin, int output_suffix_pinyin_capacity,
    wchar_t* output_suffix_path, int output_suffix_path_capacity,
    float* output_confidence, wchar_t* error_text, int error_capacity) {
    SetError(error_text, error_capacity, L"");
    SetWideOutput(output_suffix_text, output_suffix_text_capacity, L"");
    SetWideOutput(output_suffix_pinyin, output_suffix_pinyin_capacity, L"");
    SetWideOutput(output_suffix_path, output_suffix_path_capacity, L"");
    try {
        ...
        const std::wstring wide_text = Utf8TextToWide(text);
        SetWideOutput(output_suffix_text, output_suffix_text_capacity, wide_text);
        SetWideOutput(output_suffix_pinyin, output_suffix_pinyin_capacity,
            Utf8TextToWide(pinyin));
        SetWideOutput(output_suffix_path, output_suffix_path_capacity, wide_text);
        *output_confidence = confidence;
        return 1;
    } catch (const Ort::Exception& error) {
        SetError(error_text, error_capacity, Utf8ToWide(error.what()));
...
extern "C" __declspec(dllexport) void __cdecl nc_lcg_destroy(
    void* opaque_handle) {
