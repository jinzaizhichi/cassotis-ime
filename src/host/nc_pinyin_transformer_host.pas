...
type
    ...
    private type
        ...
        TncPgCreate = function(const model_path: PWideChar;
            const allowed_path: PWideChar; const intra_threads: Integer;
        ...
        TncPgRun = function(const handle: Pointer;
            ...
            const output_count: PInteger; const error_text: PWideChar;
            const error_capacity: Integer): Integer; cdecl;
...
begin
    append_log_line_shared(get_default_log_path,
        ...
        '] pinyin-transformer ' + message_text + sLineBreak);
end;
...
begin
    ...
    root_value := TJSONObject.ParseJSONValue(
        TFile.ReadAllText(vocab_path, TEncoding.UTF8));
    try
        ...
        vocab_object := root_object.GetValue('char') as TJSONObject;
        if vocab_object = nil then
        begin
            Exit;
        end;
        for pair in vocab_object do
        begin
            if TryStrToInt(pair.JsonValue.Value, index) then
            begin
                m_char_vocab.AddOrSetValue(pair.JsonString.Value, index);
                if index >= Length(m_chars_by_id) then
                begin
                    SetLength(m_chars_by_id, index + 1);
                end;
                m_chars_by_id[index] := pair.JsonString.Value;
            end;
        end;
        vocab_object := root_object.GetValue('pinyin') as TJSONObject;
        if vocab_object = nil then
        begin
            Exit;
        end;
        ...
        begin
            if TryStrToInt(pair.JsonValue.Value, index) then
            begin
                m_pinyin_vocab.AddOrSetValue(pair.JsonString.Value, index);
            end;
        end;
...
var
    ...
    vocab_path: string;
    module_local: HMODULE;
    runtime_module_local: HMODULE;
    provider_module_local: HMODULE;
...
begin
    module_local := 0;
    runtime_module_local := 0;
    provider_module_local := 0;
    session_local := nil;
    ...
    try
        ...
        generator_model_path := TPath.Combine(TPath.Combine(m_base_directory,
            'pinyin_transformer'), 'pinyin_parallel_generator_int8.onnx');
        generator_allowed_path := TPath.Combine(TPath.Combine(m_base_directory,
        ...
        { Preload the exact runtime by absolute path. Otherwise Windows can
          bind the wrapper to an unrelated onnxruntime.dll found on PATH. }
        provider_module_local := LoadLibraryEx(PChar(provider_path), 0,
            LOAD_WITH_ALTERED_SEARCH_PATH);
        if provider_module_local = 0 then
        begin
            raise EOSError.CreateFmt('LoadLibrary failed (%d): %s',
                [GetLastError, provider_path]);
        end;
        runtime_module_local := LoadLibraryEx(PChar(runtime_path), 0,
            LOAD_WITH_ALTERED_SEARCH_PATH);
        if runtime_module_local = 0 then
        begin
            raise EOSError.CreateFmt('LoadLibrary failed (%d): %s',
                [GetLastError, runtime_path]);
        end;
        module_local := LoadLibraryEx(PChar(wrapper_path), 0,
            LOAD_WITH_ALTERED_SEARCH_PATH);
        ...
        generator_create_local := TncPgCreate(GetProcAddress(module_local,
            PAnsiChar(AnsiString('nc_pg_create'))));
        generator_run_local := TncPgRun(GetProcAddress(module_local,
            PAnsiChar(AnsiString('nc_pg_run'))));
        generator_destroy_local := TncPgDestroy(GetProcAddress(module_local,
            PAnsiChar(AnsiString('nc_pg_destroy'))));
        FillChar(error_buffer, SizeOf(error_buffer), 0);
        session_local := create_local(PChar(model_path), c_model_threads,
        ...
        begin
            ...
            generator_session_local := generator_create_local(
                PChar(generator_model_path), PChar(generator_allowed_path),
        ...
        try
            ...
            module_local := 0;
            m_onnx_runtime_module := runtime_module_local;
            runtime_module_local := 0;
            m_onnx_provider_module := provider_module_local;
            provider_module_local := 0;
        ...
        begin
            ...
            if runtime_module_local <> 0 then
            begin
                FreeLibrary(runtime_module_local);
            end;
            if provider_module_local <> 0 then
            begin
                FreeLibrary(provider_module_local);
            end;
            m_state_lock.Acquire;
            try
                m_ready := False;
...
begin
    ...
    begin
        ...
        path_parts := candidates[candidate_index].segment_path.Split([#3]);
        joined_path := '';
...
begin
    ...
    try
        ...
        begin
            ...
            try
                m_generator_run_function := nil;
                m_last_error := string(PWideChar(@error_buffer[0]));
            finally
                m_state_lock.Release;
            end;
