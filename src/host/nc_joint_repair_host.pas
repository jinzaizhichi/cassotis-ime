...
begin
    ...
    vocabulary := TJSONObject.ParseJSONValue(TFile.ReadAllText(
        TPath.Combine(directory, 'local_repair/vocab.json'), TEncoding.UTF8)) as TJSONObject;
    try
        SetLength(m_inverse, vocabulary.GetValue<TJSONObject>('char').Count);
        for pair in vocabulary.GetValue<TJSONObject>('char') do
        begin
            index := StrToInt(pair.JsonValue.Value);
            m_vocab.Add(pair.JsonString.Value, index);
            m_inverse[index] := pair.JsonString.Value;
        end;
        for pair in vocabulary.GetValue<TJSONObject>('pinyin') do
            m_pinyin.Add(pair.JsonString.Value, StrToInt(pair.JsonValue.Value));
...
var
    ...
    begin Result := (last - first) * 1000.0 / TStopwatch.Frequency; end;
...
begin
    ...
    total_start := TStopwatch.GetTimeStamp;
    try
    ...
    if m_query(m_handle, @draft_ids[0], @py_ids[0], n, @logits[0], @reading_ids[0]) <> 1 then
        Exit;
    finish := TStopwatch.GetTimeStamp;
    ...
    try
        aligned := string.Join(#3, syllables);
        for i := 0 to n - 1 do
        begin
        ...
        begin
            ...
            expanded.Sort(TComparer<TBeam>.Construct(function(const a, b: TBeam): Integer
                begin
        ...
        states.Sort(TComparer<TBeam>.Construct(function(const a, b: TBeam): Integer
            begin
                if a.score > b.score then Exit(-1);
                if a.score < b.score then Exit(1);
                Result := CompareStr(a.text, b.text);
        ...
        ordered.Sort(TComparer<TChoice>.Construct(function(const a, b: TChoice): Integer
            begin
                if a.gain > b.gain then Exit(-1);
                if a.gain < b.gain then Exit(1);
                Result := CompareStr(a.text, b.text);
        ...
        lm_texts := TArray<string>.Create(draft) + texts.ToArray;
        ...
        begin
            ...
            Result.final_path_ms := elapsed(stage, TStopwatch.GetTimeStamp);
        ...
        Result.total_ms := elapsed(total_start, TStopwatch.GetTimeStamp);
    end;
end;
