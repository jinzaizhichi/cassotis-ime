...
Cassotis v1.11.0 扩展语料训练得到的词级转移覆盖，在 LM 证据充分时改善短词组合与长句路径选择；生成的组合候选使用主题强调色显示，并且不会写入用户词库。
...
`v1.11.0` 的常规数字采用“他/她在相同位置视为等价”的新评分口径，斜体数字为严格区分“他/她”的旧口径结果。`v1.10.0` 及更早版本均采用旧口径；后续版本将统一采用新口径，不再重复测试或公布旧口径数据。
