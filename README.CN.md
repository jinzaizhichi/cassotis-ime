...
Cassotis v1.9.0 为长句新增语料训练驱动的统一完整候选池。引擎会从词库约束的 lattice、具有 LM 证据的完整词组组合、局部修复和专名路径中，保留最多 32 条切分结构多样的完整候选，再结合词路径与字符 LM、切分质量、专名置信度、N-best 共识以及离线训练的成对与残差分数统一排序。证据不足时由置信门控保留原有顺序，并通过共享缓存和时间预算控制扩大召回带来的开销。在长句基准测试-16300中，Top1 从 50.83% 提升至 55.96%，Top2 从 55.63% 提升至 65.55%；Mean/P95 延迟则从 72.44/281 ms 降至 59.65/187 ms。
